  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        <StatCard label="Registered donors" value={stats?.donorCount ?? 0} />
        <StatCard label="Open emergencies" value={stats?.openRequests ?? 0} />
        <StatCard label="Low-stock groups" value={stats?.lowStockGroups.length ?? 0} />
      </div>
              ...
              lowStock.map((item) => (
                <Card key={item.group} className="flex-row items-center justify-between gap-3 p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{item.units} units across all blood banks.</p>
                  </div>
